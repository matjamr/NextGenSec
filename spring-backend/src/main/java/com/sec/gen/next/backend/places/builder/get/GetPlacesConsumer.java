        ...
        if(placesContext.getUserScope()) {
            return ret.stream()
                    .filter(place -> place.getAuthorizedUsers()
                            .stream()
                            .anyMatch(userPlaceAssignmentModel -> userPlaceAssignmentModel.getUser().getEmail().equals(placesContext.getAuthorizedUser().getName())))
                    .map(place -> place.toBuilder()
                            .authorizedUsers(place.getAuthorizedUsers().stream().filter(user -> user.getUser().getEmail().equals(placesContext.getAuthorizedUser().getName())).toList())
                            .build())
                    .toList();
        }
